...
corre_female_ave=zeros(N_res,N_window-2);  % correlation of community memberships of the neighbouring age groups of female
corre_male_ave=zeros(N_res,N_window-2);  % correlation of community memberships of the neighbouring age groups of male
...
for i=1:N_res  % modularity resolution
    ...
    if scan==1
        for n=1:N_window-2
            ...
            corre_female_ave(i,n)=jaccardsimi(label_AP_F_ave(:,n),label_AP_F_ave(:,n+2));
            corre_male_ave(i,n)=jaccardsimi(label_AP_M_ave(:,n),label_AP_M_ave(:,n+2));
        end
    elseif scan==2
        for n=1:N_window-2
         
            corre_female_ave(i,n)=jaccardsimi(label_PA_F_ave(:,n),label_PA_F_ave(:,n+2));
            corre_male_ave(i,n)=jaccardsimi(label_PA_M_ave(:,n),label_PA_M_ave(:,n+2));
        end
    end
    
    %plot(1:(N_window-1),corre_hier,'r')

    plot(1:(N_window-2),corre_female_ave(i,:),'color',[1,0.87,0.68],...
                 'LineWidth',1);
  
    plot(1:(N_window-2),corre_male_ave(i,:),'color',[0.75,0.75,0.75],...
                 'LineWidth',1);
...
plot(1:(N_window-2),corre_male_ave_mean(1,:),'color',[0,0,0.],...
                 'LineWidth',2);

corre_female_ave_mean=mean(corre_female_ave);
plot(1:(N_window-2),corre_female_ave_mean(1,:),'color',[0.82,0.41,0.12],...
                 'LineWidth',2);

legend([plot(1:(N_window-2),corre_female_ave_mean(1,:),'color',[0.82,0.41,0.12],...
                 'LineWidth',2),plot(1:(N_window-2),corre_male_ave_mean(1,:),'color',[0,0,0.],...
...
if scan==1
    title(['Development of modularity (','AP',', ROI=',num2str(N_roi),')'],'fontsize', 16)
else
    title(['Development of modularity (','PA',', ROI=',num2str(N_roi),')'],'fontsize', 16)
end
