% DEMON Mean community size versus resolution
%
%
% Version 1.0
% 7-Dec-2023
...
size_Bayes=zeros(N_window,N_res);
size_ave=zeros(N_window,N_res);
size_indi=cell(N_window,1);
for n=1:N_window
    [size_Bayes(n,:),size_ave(n,:),size_indi{n,1}]=mean_community_size(scan,N_roi,N_res,n);
end
...
for j=1:N_res
    count_size=tabulate(size_indi{1,1}(:,j));
    count_size_og=count_size;
    count_size(count_size(:,2)==0,:)=[];
    scatter(res(j),count_size(:,1),20*count_size(:,2),'filled')
    alpha(0.6)
    hold on
end
...
if scan==1
    ...
    saveas(gcf,['../figures/','roi_',num2str(N_roi),'_AP','_community_size.fig'])
    saveas(gcf,['../figures_paper/','roi_',num2str(N_roi),'_AP','_community_size.png'])
elseif scan==2
  
    saveas(gcf,['../figures/','roi_',num2str(N_roi),'_PA','_community_size.fig'])
    saveas(gcf,['../figures_paper/','roi_',num2str(N_roi),'_PA','_community_size.png'])
end
...
% -------------------------------------------------------------------------
% nested function
function [size,size_ave,size_indi]=mean_community_size(scan,N_roi,N_res,age_window)
% scan: 1 AP, 2 PA
% N_roi: number of ROIs
% N_res: number of modularity resolutions    
% age_window: age window number

    size=zeros(1,N_res);  % mean community size estimated using hierarchical Bayesian modelling
    size_ave=zeros(1,N_res);  % mean community size using group averaged FC
    ...
    if scan==1
        ...
        for j=1:N_res
            ...
            size(j)=community_size(label_AP(:,age_window));
            load(['../results/','roi_',num2str(N_roi),'_1_','AP','/',num2str(res(j)),'/labels_AP_ave.mat']); 
            size_ave(j)=community_size(label_AP_ave(:,age_window));
        end
        ...
        for j=1:N_res
            ...
            size(j)=community_size(label_PA(:,age_window));
            load(['../results/','roi_',num2str(N_roi),'_1_','PA','/',num2str(res(j)),'/labels_PA_ave.mat']); 
            size_ave(j)=community_size(label_PA_ave(:,age_window));
        end
    ...
    size_indi=zeros(label_res{1,1}.count_subj(1,age_window),N_res); % mean community size of individual FC
    for j=1:N_res
        for i=1:label_res{1,1}.count_subj(1,age_window)
            size_indi(i,j)=community_size(label_res{1,j}.label{i,age_window});
        end
    end
end
