% DEMO Statistical analysis: modular similarity of awake and sleeping (comparision between male and female)
...
if scan==1
    load(['/home/lingbin/Documents/work/pythonProject/BCP_fMRI/results_F/roi_',num2str(N_roi),'_awake_sleep_AP.mat']);
    load(['/home/lingbin/Documents/work/pythonProject/BCP_fMRI/results_M/roi_',num2str(N_roi),'_awake_sleep_AP.mat']);
else 
    load(['/home/lingbin/Documents/work/pythonProject/BCP_fMRI/results_F/roi_',num2str(N_roi),'_awake_sleep_PA.mat']);
    load(['/home/lingbin/Documents/work/pythonProject/BCP_fMRI/results_M/roi_',num2str(N_roi),'_awake_sleep_PA.mat']);
end

age_range={'0-5 Month', '3-8 Month', '6-11 Month','9-14 Month','12-17 Month','15-23 Month','18-29 Month','24-36 Month'};
...
fillcolors=[repmat(fillcolor1,8,1);repmat(fillcolor2,8,1)];
position_1 = 0.8:1:7.8;  % define position for first group boxplot
position_2 = 1.2:1:8.2;  % define position for second group boxplot 
...
set(gca,'XTick', [1 2 3 4 5 6 7 8],'Xlim',[0 9]);
boxchi = get(gca, 'Children');


%box_var=findall(gca,'Tag','Box')
%legend(box_var([7,14]), {'Hierarchical Bayesian', 'Group Averaging'});
hLegend=legend([boxchi(8),boxchi(9)], ["Female", "Male"] ,'location','southeast');

legendEntries = hLegend.EntryContainer;
legendEntries.Children(3:end) = [];
...
set(gcf,'unit','centimeters','position',[6 10 16 14])
...
if scan==1
    results_path=fullfile(data_path,['../results/','roi_',num2str(N_roi),'_AP','_statistical_analysis_awake_sleep']);
    save(results_path); 
    saveas(gcf,['../figures/','roi_',num2str(N_roi),'_AP','_statistical_analysis_awake_sleep.fig'])
elseif scan==2
    results_path=fullfile(data_path,['../results/','roi_',num2str(N_roi),'_PA','_statistical_analysis_awake_sleep']);
    save(results_path); 
    saveas(gcf,['../figures/','roi_',num2str(N_roi),'_PA','_statistical_analysis_awake_sleep.fig'])
end
